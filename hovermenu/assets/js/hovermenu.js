    ...
    var hovermenu = {
        ...
        setButtons: function(buttonsObj) {
            for (var i in buttonsObj) {
                hovermenu.menu.children(_.strFormat('.{{pos}}', {
                    pos: i
                })).children('.hm-text')
                    .html(_.strFormat('<i class="{{icon}}"></i><span>{{text}}</span>', {
...
$(function() {
    $.hovermenu('init', {
        buttons: {
            ...
            "bl": {
                ...
                text: "HOME",
                callback: function() {
                    window.location = "http://alexcheuk.com";
                }
            },
            "br": {
                icon: "icon-github",
                text: "FORK",
                callback: function() {
                    window.open("https://github.com/alexcheuk/hovermenu", "_blank");
                }
            }
        }
    });
    ...
    function showAction(txt) {
        $('.action').stop().css("opacity", "1").text(txt).show().fadeOut(3000, function() {
            $(this).text("PRESS CTRL OR CLICK ANY WHERE ON THE PAGE!").css("opacity", "1").show();
        });
    }
}) 